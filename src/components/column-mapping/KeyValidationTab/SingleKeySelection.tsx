...
interface SingleKeySelectionProps {
  allColumns: Column[];
  selectedIds: string[];
...
const SingleKeySelection: React.FC<SingleKeySelectionProps> = ({ allColumns, selectedIds, onSelectionChange }) => {
  ...
  return (
    <Card>
      <CardHeader className="p-3">
        <CardTitle className="text-sm font-medium">Select & Reorder Key Columns</CardTitle>
      </CardHeader>
          ...
          <PopoverTrigger asChild>
            <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between">
              <span className="truncate">
                {selectedColumns.length > 0 ? `${selectedColumns.length} keys selected` : "Select key columns..."}
              </span>
...
export default SingleKeySelection;
